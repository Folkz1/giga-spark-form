...
import { Rocket, TrendingUp, Brain, BarChart2, MessageSquareText, FileBarChart, ArrowRight, Settings2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] flex flex-col items-center px-6 lg:px-8 pt-10 pb-8">
      <div className="w-full max-w-[1200px]">
        ...
        {/* ===== TRACKING API ===== */}
        <div className="mt-6">
          <SectionDivider label="Tracking API" />
          <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4">
            <motion.button
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              className="group relative rounded-xl p-5 text-left bg-slate-800/40 border border-slate-700/40 transition-all duration-200 cursor-pointer hover:translate-y-[-2px] hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/5"
            >
              <ArrowRight className="absolute top-4 right-4 w-4 h-4 text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
              <div className="w-11 h-11 rounded-lg flex items-center justify-center mb-3 bg-amber-500/10">
                <Settings2 className="w-[22px] h-[22px] text-amber-400" />
              </div>
              <h2 className="text-base font-semibold text-white mb-1">Configurações</h2>
              <p className="text-sm text-slate-400 leading-relaxed">Clientes, pixels Meta/Google, mapeamentos CRM</p>
            </motion.button>
          </div>
        </div>
